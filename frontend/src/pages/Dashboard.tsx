    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-lg border p-5 mb-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
          <Zap size={16} className="text-amber-500" /> 量化价值（基于模拟案例估算）
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div className="flex items-start gap-2">
            <span className="text-green-500 font-bold">↓</span>
            <div>初步读标：<span className="font-medium">2-4小时 → 10-20分钟</span></div>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-green-500 font-bold">↓</span>
            <div>资格整理：<span className="font-medium">1小时 → 5分钟</span></div>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-green-500 font-bold">↓</span>
            <div>评分分析：<span className="font-medium">1-2小时 → 10分钟</span></div>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-green-500 font-bold">↓</span>
            <div>响应表初稿：<span className="font-medium">半天 → 10-15分钟</span></div>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-green-500 font-bold">✓</span>
            <div>废标风险覆盖：<span className="font-medium">报价/资质/签章/保证金/★号参数</span></div>
          </div>
          <div className="flex items-start gap-2">
            <span className="text-green-500 font-bold">✓</span>
            <div>新人辅助：<span className="font-medium">降低对老员工经验依赖</span></div>
          </div>
        </div>
      </div>
      ...
      {/* 示例项目 */}
      <div className="bg-white rounded-lg border p-5 mb-6">
        <h3 className="text-sm font-semibold text-gray-700 mb-3">示例招标项目</h3>
        <div className="space-y-3">
          {samples.map((s) => (
            <div key={s.id} className="flex items-center justify-between p-3 rounded border hover:bg-gray-50">
              <div>
                <div className="text-sm font-medium text-gray-800">{s.name}</div>
                <div className="text-xs text-gray-500 mt-0.5">
                  {s.industry} · 预算{s.budget} · {s.description}
                </div>
              </div>
              <button
                onClick={() => navigate('/analysis', { state: { tenderId: s.id } })}
                className="text-xs bg-primary-600 text-white px-4 py-1.5 rounded hover:bg-primary-700 flex-shrink-0"
              >
                一键分析
              </button>
            </div>
